...
// Built: 2026-05-03 (v16 — push notifications, forgot password, photo compression, birthday/digest emails, iCal, KK UI, custom branding, admin member management, delete comments)
...
async function sendWebPush(env, subscription, title, body, url='/') {
  ...
  try {
    ...
    const p256dh = b64urlToUint8(subscription.p256dh);
    const auth = b64urlToUint8(subscription.auth);
    ...
    const ecdhPubRaw = await crypto.subtle.exportKey('raw', ecdh.publicKey);
    ...
    // ECDH shared secret
    const sharedBits = await crypto.subtle.deriveBits({name:'ECDH',public:receiverKey}, ecdh.privateKey, 256);

    // HKDF PRK using auth as salt
    const prkKey = await crypto.subtle.importKey('raw', sharedBits, {name:'HKDF'}, false, ['deriveBits']);
    const authInfo = new TextEncoder().encode('Content-Encoding: auth\0');
    const prk = await crypto.subtle.deriveBits({name:'HKDF',hash:'SHA-256',salt:auth,info:authInfo}, prkKey, 256);

    // Content encryption key + nonce
    const salt = crypto.getRandomValues(new Uint8Array(16));
    const prkKey2 = await crypto.subtle.importKey('raw', prk, {name:'HKDF'}, false, ['deriveBits']);
    const context = new Uint8Array([...new TextEncoder().encode('P-256\0'), 0, 65, ...new Uint8Array(ecdhPubRaw), 0, 65, ...p256dh]);
    ...
    const cekBits = await crypto.subtle.deriveBits({name:'HKDF',hash:'SHA-256',salt,info:cekInfo}, prkKey2, 128);
    ...
    const claims = uint8ToB64url(new TextEncoder().encode(JSON.stringify({aud:origin,exp:now+43200,sub:'mailto:paddy@luckdragon.io'})));
    const sigInput = new TextEncoder().encode(`${header}.${claims}`);
    const vapidKey = await crypto.subtle.importKey('pkcs8', b64urlToUint8(env.VAPID_PRIVATE_KEY), {name:'ECDSA',namedCurve:'P-256'}, false, ['sign']);
    const sig = await crypto.subtle.sign({name:'ECDSA',hash:'SHA-256'}, vapidKey, sigInput);
    const jwt = `${header}.${claims}.${uint8ToB64url(new Uint8Array(sig))}`;
    ...
    await fetch(endpoint, {
      ...
      headers:{
        ...
        'Crypto-Key':`dh=${uint8ToB64url(new Uint8Array(ecdhPubRaw))};p256ecdsa=${env.VAPID_PUBLIC_KEY}`,
        'Authorization':`vapid t=${jwt},k=${env.VAPID_PUBLIC_KEY}`,
        'TTL':'86400'
      },
      body: encrypted
    });
  } catch(e) { /* push errors should not break the request */ }
}

async function sendPushToUser(env, userId, title, body, url='/') {
  try {
    const subs = await env.DB.prepare('SELECT * FROM push_subscriptions WHERE user_id=?').bind(userId).all();
    for (const sub of subs.results||[]) {
      await sendWebPush(env, {endpoint:sub.endpoint,p256dh:sub.p256dh,auth:sub.auth}, title, body, url);
    }
  } catch {}
}
...
async function createNotif(env, userId, type, title, body, refId=null, refType=null) {
  try {
    ...
    // Fire web push
    await sendPushToUser(env, userId, title, body);
  } catch {}
}
...
async function handleAuth(path, request, env) {
  ...
  if (path === '/api/auth/forgot-password' && method === 'POST') {
    ...
    const user = await env.DB.prepare('SELECT * FROM users WHERE LOWER(email)=LOWER(?)').bind(email.trim()).first();
    // Always return ok to avoid email enumeration
    if (user && env.RESEND_API_KEY) {
      const token = crypto.randomUUID();
      await env.DB.prepare('INSERT INTO password_resets (token,user_id,expires_at) VALUES (?,?,datetime("now","+1 hour"))').bind(token, user.id).run();
      const link = `https://hub.luckdragon.io/?reset=${token}`;
      await fetch('https://api.resend.com/emails', {
        method:'POST', headers:{'Authorization':`Bearer ${env.RESEND_API_KEY}`,'Content-Type':'application/json'},
        body:JSON.stringify({from:'Family Hub <paddy@luckdragon.io>',to:[email],subject:'Reset your Family Hub password',
          html:`<p>Hi ${user.name}! Click below to reset your password. Link expires in 1 hour.</p><p><a href="${link}" style="background:#6366f1;color:white;padding:12px 24px;border-radius:8px;text-decoration:none;display:inline-block">Reset Password →</a></p><p style="color:#888;font-size:12px">If you didn't request this, ignore this email.</p>`
        })
      });
    }
    return json({ok:true});
  ...
  if (path === '/api/auth/reset-password' && method === 'POST') {
    ...
    if (!token || !password || password.length < 6) return err('Invalid request');
...
async function handlePosts(path, request, env, user) {
  ...
  const deleteCommentMatch = path.match(/^\/api\/posts\/([^/]+)\/comments\/([^/]+)$/);
  if (deleteCommentMatch && method === 'DELETE') {
    ...
    if (comment.user_id !== user.id && user.role !== 'admin') return err('Forbidden', 403);
...
<div class="app" id="app">
  ...
  <div class="auth-screen" id="authScreen">
    ...
    <div class="auth-card">
      ...
      <div id="loginForm">
        ...
        <p style="text-align:center;margin-top:12px;font-size:13px;color:var(--muted)"><a href="#" onclick="showForgotPassword()" style="color:var(--primary)">Forgot password?</a></p>
      </div>
    ...
    <div class="screen" id="screenEvents">
      <div class="header">
        ...
        <div style="display:flex;gap:6px">
          <a href="/api/events/ical" title="Subscribe to calendar" style="padding:6px 10px;border-radius:8px;border:1px solid var(--border);color:var(--muted);font-size:12px;text-decoration:none;display:flex;align-items:center;gap:4px" download>📆 iCal</a>
  ...
  <div id="familySettingsScreen">
    ...
    <div style="padding:16px;max-width:520px;margin:0 auto">
      ...
      <div style="background:var(--surface);border-radius:12px;padding:16px;margin-bottom:14px">
        <h3 style="font-size:15px;margin:0 0 12px">🎨 Custom Branding</h3>
        <div style="margin-bottom:12px">
          <label style="display:block;font-size:12px;color:var(--muted);margin-bottom:6px">Family Logo</label>
          <div style="display:flex;align-items:center;gap:10px">
            <img id="brandingLogoPreview" style="width:56px;height:56px;border-radius:12px;object-fit:cover;display:none;border:2px solid var(--border)">
            <div id="brandingLogoPlaceholder" style="width:56px;height:56px;border-radius:12px;background:var(--surface2);display:flex;align-items:center;justify-content:center;font-size:24px;border:2px dashed var(--border)">🏠</div>
            <label class="btn btn-sm" style="cursor:pointer">Upload Logo<input type="file" accept="image/*" style="display:none" onchange="uploadFamilyLogo(this)"></label>
          </div>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px">
          <div>
            <label style="display:block;font-size:12px;color:var(--muted);margin-bottom:6px">Primary Colour</label>
            <div style="display:flex;align-items:center;gap:8px">
              <input type="color" id="brandingPrimaryColor" value="#6366f1" style="width:40px;height:36px;border:none;border-radius:8px;cursor:pointer;background:none" oninput="document.getElementById('brandingPrimaryHex').value=this.value">
              <input type="text" id="brandingPrimaryHex" value="#6366f1" style="flex:1;padding:8px;background:var(--surface2);border:1px solid var(--border);border-radius:6px;color:var(--text);font-size:13px" oninput="document.getElementById('brandingPrimaryColor').value=this.value">
            </div>
          </div>
          <div>
            <label style="display:block;font-size:12px;color:var(--muted);margin-bottom:6px">Accent Colour</label>
            <div style="display:flex;align-items:center;gap:8px">
              <input type="color" id="brandingAccentColor" value="#818cf8" style="width:40px;height:36px;border:none;border-radius:8px;cursor:pointer;background:none" oninput="document.getElementById('brandingAccentHex').value=this.value">
              <input type="text" id="brandingAccentHex" value="#818cf8" style="flex:1;padding:8px;background:var(--surface2);border:1px solid var(--border);border-radius:6px;color:var(--text);font-size:13px" oninput="document.getElementById('brandingAccentColor').value=this.value">
            </div>
          </div>
        </div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px">
          <span style="font-size:12px;color:var(--muted);align-self:center">Presets:</span>
          ${['#6366f1,#818cf8','#10b981,#34d399','#ef4444,#f87171','#f59e0b,#fbbf24','#ec4899,#f472b6','#06b6d4,#22d3ee'].map(c=>{const[p,a]=c.split(',');return `<button onclick="setBrandingPreset('${p}','${a}')" style="width:28px;height:28px;border-radius:50%;background:${p};border:2px solid rgba(255,255,255,.2);cursor:pointer"></button>`}).join('')}
        </div>
...
// Init service worker + handle password reset URL
initServiceWorker();
const _resetToken = new URLSearchParams(location.search).get('reset');
if (_resetToken) { handlePasswordReset(_resetToken); }
...
function _renderComments(comments) {
  ...
  return comments.map(c => \`<div class="comment-bubble">
    ...
    <div class="comment-bubble-body" style="flex:1">
    ...
    \${(c.user_id===currentUser?.id||currentUser?.role==='admin')?\`<button onclick="deleteComment('\${commentPostId}','\${c.id}')" style="background:none;border:none;color:var(--muted);font-size:16px;cursor:pointer;padding:4px;flex-shrink:0">🗑</button>\`:''}
  </div>\`).join('');
}
async function deleteComment(postId, commentId) {
  if (!confirm('Delete this comment?')) return;
  const r = await api(\`/api/posts/\${postId}/comments/\${commentId}\`, {method:'DELETE'});
  if (r.ok) { const c = await api(\`/api/posts/\${postId}/comments\`); document.getElementById('commentList').innerHTML = _renderComments(c); }
  else toast('❌ '+r.error);
}
      ...
      <div style="margin-top:14px;padding-top:14px;border-top:1px solid var(--border)">
        <div style="display:flex;align-items:center;justify-content:space-between">
          <div>
            ...
            <div style="font-size:12px;color:var(--muted)" id="pushStatusLabel">Enable browser push alerts</div>
          </div>
          <button id="pushToggleBtn" onclick="togglePushSubscription()" class="btn btn-sm btn-primary">Enable</button>
        </div>
      </div>
    </div>
...
async function checkPushStatus() {
  if (!('serviceWorker' in navigator) || !('PushManager' in window)) {
    ...
    const lbl = document.getElementById('pushStatusLabel');
    if (btn) btn.style.display = 'none';
    if (lbl) lbl.textContent = 'Not supported in this browser';
    return;
  }
  try {
    ...
    const lbl = document.getElementById('pushStatusLabel');
    if (pushSubscription) {
      if (btn) { btn.textContent = 'Disable'; btn.className = 'btn btn-sm'; }
      if (lbl) lbl.textContent = '✅ Push notifications are enabled';
    } else {
      if (btn) { btn.textContent = 'Enable'; btn.className = 'btn btn-sm btn-primary'; }
      if (lbl) lbl.textContent = 'Enable browser push alerts';
    }
  } catch(e) {}
}
...
async function togglePushSubscription() {
  ...
  try {
    ...
    if (pushSubscription) {
      ...
      await api('/api/push/subscribe', {method:'DELETE', body:JSON.stringify({endpoint:pushSubscription.endpoint})});
      pushSubscription = null;
      toast('Push notifications disabled');
    } else {
      const {publicKey} = await api('/api/push/vapid-key');
      if (!publicKey) { toast('Push not configured'); return; }
      const sub = await reg.pushManager.subscribe({userVisibleOnly:true, applicationServerKey:publicKey});
      const {endpoint, keys} = sub.toJSON();
      await api('/api/push/subscribe', {method:'POST', body:JSON.stringify({endpoint, p256dh:keys.p256dh, auth:keys.auth})});
...
function showForgotPassword() {
  ...
  fetch('/api/auth/forgot-password', {method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({email})})
    .then(r=>r.json()).then(()=> toast('📧 If that email exists, a reset link has been sent!'));
}

async function handlePasswordReset(token) {
  const pw = prompt('Enter your new password (min 6 characters):');
  if (!pw || pw.length < 6) { toast('Password too short'); return; }
  const pw2 = prompt('Confirm new password:');
  if (pw !== pw2) { toast('Passwords do not match'); return; }
  const r = await fetch('/api/auth/reset-password', {method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({token, password:pw})}).then(r=>r.json());
  if (r.ok) { toast('✅ Password reset! Please log in.'); window.history.replaceState(null,'','/'); }
  else toast('❌ ' + (r.error||'Reset failed'));
}

// ─── CUSTOM BRANDING ──────────────────────────────────────────────────────────
async function loadBranding(fam) {
  if (!fam) return;
  const primary = fam.primary_color || '#6366f1';
  const accent = fam.accent_color || '#818cf8';
  document.documentElement.style.setProperty('--primary', primary);
  document.documentElement.style.setProperty('--primary-dark', accent);
  // Update branding inputs if settings screen is open
  const pc = document.getElementById('brandingPrimaryColor');
  const ac = document.getElementById('brandingAccentColor');
  const ph = document.getElementById('brandingPrimaryHex');
  const ah = document.getElementById('brandingAccentHex');
  ...
  if (fam.logo_key) {
    ...
    if (preview) { preview.src = '/api/photos/'+fam.logo_key; preview.style.display='block'; document.getElementById('brandingLogoPlaceholder').style.display='none'; }
  }
}

function setBrandingPreset(primary, accent) {
  const pc = document.getElementById('brandingPrimaryColor');
  const ac = document.getElementById('brandingAccentColor');
  const ph = document.getElementById('brandingPrimaryHex');
  const ah = document.getElementById('brandingAccentHex');
...
async function saveBranding() {
  ...
  if (!fam) return;
  const primary_color = document.getElementById('brandingPrimaryColor')?.value || '#6366f1';
  const accent_color = document.getElementById('brandingAccentColor')?.value || '#818cf8';
  await api('/api/families/'+fam.id, {method:'PATCH', body:JSON.stringify({primary_color, accent_color})});
  document.documentElement.style.setProperty('--primary', primary_color);
  document.documentElement.style.setProperty('--primary-dark', accent_color);
  currentFamily = null;
  toast('🎨 Branding saved!');
}

async function uploadFamilyLogo(input) {
  const fam = await ensureFamily();
  if (!fam || !input.files[0]) return;
  const fd = new FormData();
  fd.append('file', input.files[0]);
  const headers = {};
  if (session?.token) headers['x-session-token'] = session.token;
  const r = await fetch('/api/families/'+fam.id+'/logo', {method:'POST', headers, body:fd}).then(r=>r.json());
  if (r.ok) {
    toast('Logo uploaded! ✅');
    const preview = document.getElementById('brandingLogoPreview');
    const placeholder = document.getElementById('brandingLogoPlaceholder');
    if (preview) { preview.src = '/api/photos/'+r.key; preview.style.display='block'; }
...
async function toggleMemberRole(memberId, currentRole) {
  ...
  if (memberId === currentUser?.id) { toast("Can't change your own role"); return; }
  const newRole = currentRole === 'admin' ? 'member' : 'admin';
  const r = await api('/api/admin/members/'+memberId, {method:'PATCH', body:JSON.stringify({role:newRole})});
  if (r.ok) { toast(newRole==='admin'?'Made admin ✅':'Removed admin ✅'); openFamilySettings(); }
  else toast('❌ '+r.error);
}
...
async function openFamilySettings() {
  ...
  document.getElementById('familyMembersList').innerHTML = members.map(m=>\`
    <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--border)">
      \${avatarEl(m)}
      <span style="flex:1">\${esc(m.name)}\${!m.registered?'<span style="font-size:10px;color:var(--muted);margin-left:4px">(not joined)</span>':''}</span>
      \${isAdmin && m.id !== currentUser?.id ? \`<button onclick="toggleMemberRole('\${m.id}','\${m.role||'member'}')" style="font-size:11px;background:\${m.role==='admin'?'var(--primary)':'var(--surface2)'};color:#fff;padding:3px 10px;border-radius:99px;border:none;cursor:pointer">\${m.role||'member'}</button>\` : \`<span style="font-size:11px;background:\${m.role==='admin'?'var(--primary)':'var(--surface2)'};color:#fff;padding:2px 8px;border-radius:99px">\${m.role||'member'}</span>\`}
    </div>\`).join('');
}
...
export default {
  async scheduled(event, env, ctx) {
    // Birthday check — runs daily
    try {
      const today = new Date();
      const mm = String(today.getMonth()+1).padStart(2,'0');
      const dd = String(today.getDate()).padStart(2,'0');
      const birthdays = await env.DB.prepare(
        "SELECT b.*, u.name as person_name, fm.user_id as member_id FROM birthdays b JOIN users u ON u.id=b.person_id LEFT JOIN family_members fm ON fm.family_id=(SELECT family_id FROM family_members WHERE user_id=b.person_id LIMIT 1) WHERE strftime('%m-%d', b.date)=?"
      ).bind(`${mm}-${dd}`).all();
      const age = (dob) => today.getFullYear() - new Date(dob).getFullYear();
      const notified = new Set();
      for (const bday of birthdays.results||[]) {
        const msg = `🎂 Today is ${bday.person_name}'s birthday! They're turning ${age(bday.date)}.`;
        // Notify all family members
        const members = await env.DB.prepare('SELECT DISTINCT user_id FROM family_members WHERE family_id=(SELECT family_id FROM family_members WHERE user_id=? LIMIT 1)').bind(bday.person_id).all();
        for (const m of members.results||[]) {
          if (!notified.has(m.user_id+bday.person_id)) {
            await createNotif(env, m.user_id, 'birthday', `🎂 ${bday.person_name}'s Birthday!`, msg, bday.person_id, 'birthday');
            notified.add(m.user_id+bday.person_id);
          }
        }
      }
    } catch(e) {}
    ...
    try {
      if (new Date().getDay() === 0 && env.RESEND_API_KEY) {
        const weekAgo = new Date(Date.now()-7*86400000).toISOString();
        const families = await env.DB.prepare('SELECT * FROM families').all();
        for (const fam of families.results||[]) {
          const [posts, events] = await Promise.all([
            env.DB.prepare('SELECT p.content,u.name FROM posts p JOIN users u ON u.id=p.user_id WHERE p.created_at>? ORDER BY p.created_at DESC LIMIT 10').bind(weekAgo).all(),
            env.DB.prepare("SELECT title,start_time FROM events WHERE family_id=? AND start_time>? ORDER BY start_time LIMIT 5").bind(fam.id, new Date().toISOString()).all()
          ]);
          if (!posts.results?.length && !events.results?.length) continue;
          const members = await env.DB.prepare('SELECT u.email,u.name FROM users u JOIN family_members fm ON fm.user_id=u.id WHERE fm.family_id=? AND u.email IS NOT NULL').bind(fam.id).all();
          const postHtml = (posts.results||[]).map(p=>`<li><strong>${p.name}:</strong> ${p.content?.slice(0,100)||'📷 Photo'}</li>`).join('');
          const evtHtml = (events.results||[]).map(e=>`<li>${e.title} — ${new Date(e.start_time).toLocaleDateString('en-AU',{weekday:'short',day:'numeric',month:'short'})}</li>`).join('');
          for (const m of members.results||[]) {
            await fetch('https://api.resend.com/emails', {
              method:'POST', headers:{'Authorization':`Bearer ${env.RESEND_API_KEY}`,'Content-Type':'application/json'},
              body:JSON.stringify({
                from:`${fam.name} <paddy@luckdragon.io>`,to:[m.email],
                subject:`📰 ${fam.name} — Weekly Digest`,
                html:`<h2>👋 Hi ${m.name}! Here's your weekly family update.</h2>${postHtml?`<h3>📝 Posts this week</h3><ul>${postHtml}</ul>`:''}${evtHtml?`<h3>📅 Upcoming events</h3><ul>${evtHtml}</ul>`:''}<p><a href="https://hub.luckdragon.io" style="background:#6366f1;color:white;padding:10px 20px;border-radius:8px;text-decoration:none">Open Family Hub →</a></p>`
              })
            });
          }
        }
...
`ALTER TABLE invites ADD COLUMN email TEXT`,
  ...
  // v16 migrations
  const v16tables = [
    `CREATE TABLE IF NOT EXISTS password_resets (token TEXT PRIMARY KEY, user_id TEXT NOT NULL, expires_at DATETIME NOT NULL, used INTEGER DEFAULT 0, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)`,
    `CREATE TABLE IF NOT EXISTS push_subscriptions (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id TEXT NOT NULL, endpoint TEXT NOT NULL, p256dh TEXT NOT NULL, auth TEXT NOT NULL, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, UNIQUE(user_id,endpoint))`,
    `ALTER TABLE families ADD COLUMN primary_color TEXT DEFAULT '#6366f1'`,
    `ALTER TABLE families ADD COLUMN accent_color TEXT DEFAULT '#818cf8'`,
    `ALTER TABLE families ADD COLUMN theme TEXT DEFAULT 'dark'`,
  ];
  for (const sql of v16tables) { try { await env.DB.exec(sql); } catch(e) {} }
...
`;
    ...
    const authRoutes = ['/api/auth/invite','/api/auth/register','/api/auth/login','/api/auth/forgot-password','/api/auth/reset-password'];
    ...
    if (path.startsWith('/api/events')) {
      if (path === '/api/events/ical' && method === 'GET') {
        const famRow = await env.DB.prepare('SELECT family_id FROM family_members WHERE user_id=? LIMIT 1').bind(user.id).first();
        if (!famRow) return new Response('BEGIN:VCALENDAR\r\nVERSION:2.0\r\nEND:VCALENDAR', {headers:{'content-type':'text/calendar','cache-control':'no-cache'}});
        const rows = await env.DB.prepare('SELECT * FROM events WHERE family_id=? ORDER BY start_time').bind(famRow.family_id).all();
        const vevents = (rows.results||[]).map(e => {
          const dt = s => new Date(s).toISOString().replace(/[-:]/g,'').slice(0,15)+'Z';
          return 'BEGIN:VEVENT\r\nUID:'+e.id+'@familyhub\r\nSUMMARY:'+(e.title||'')+'\r\nDTSTART:'+dt(e.start_time)+'\r\nDTEND:'+dt(e.end_time||e.start_time)+'\r\nDESCRIPTION:'+(e.description||'').replace(/\n/g,'\\n')+'\r\nEND:VEVENT';
        }).join('\r\n');
        const ical = 'BEGIN:VCALENDAR\r\nVERSION:2.0\r\nPRODID:-//FamilyHub//EN\r\nCALSCALE:GREGORIAN\r\n'+vevents+'\r\nEND:VCALENDAR';
        return new Response(ical, {headers:{'content-type':'text/calendar; charset=utf-8','content-disposition':'attachment; filename="family.ics"','cache-control':'no-cache'}});
      }
    ...
    // Branding (v16)
    const famBrandingMatch = path.match(/^\/api\/families\/([^/]+)\/branding$/);
    if (famBrandingMatch && method === 'PATCH') {
      if (user.role !== 'admin') return err('Forbidden', 403);
      const fid = famBrandingMatch[1];
      const {primary_color, accent_color, theme} = await request.json();
      await env.DB.prepare('UPDATE families SET primary_color=COALESCE(?,primary_color), accent_color=COALESCE(?,accent_color), theme=COALESCE(?,theme) WHERE id=?').bind(primary_color||null,accent_color||null,theme||null,fid).run();
      return json({ok:true});
    }
    ...
    // ── ADMIN: promote/demote member (v16) ────────────────────────────────────
    const adminMemberMatch = path.match(/^\/api\/admin\/members\/([^/]+)$/);
    if (adminMemberMatch && method === 'PATCH') {
      if (user.role !== 'admin') return err('Forbidden', 403);
      const {role} = await request.json();
      if (!['admin','member'].includes(role)) return err('Invalid role');
      const famRow = await env.DB.prepare('SELECT family_id FROM family_members WHERE user_id=? LIMIT 1').bind(user.id).first();
      if (!famRow) return err('No family');
      await env.DB.prepare('UPDATE family_members SET role=? WHERE user_id=? AND family_id=?').bind(role, adminMemberMatch[1], famRow.family_id).run();
      return json({ok:true});
    }

    // ── PUSH SUBSCRIPTIONS (v16) ──────────────────────────────────────────────
    ...
    if (path === '/api/push/subscribe' && method === 'POST') {
      ...
      await env.DB.prepare('INSERT OR REPLACE INTO push_subscriptions (user_id,endpoint,p256dh,auth) VALUES (?,?,?,?)').bind(user.id,endpoint,keys.p256dh,keys.auth).run();
      return json({ok:true});
    }
    ...
    if (commentDelMatch && method === 'DELETE') {
      const comment = await env.DB.prepare('SELECT * FROM post_comments WHERE id=?').bind(commentDelMatch[2]).first();
      if (!comment) return err('Not found', 404);
      if (comment.user_id !== user.id && user.role !== 'admin') return err('Forbidden', 403);
      await env.DB.prepare('DELETE FROM post_comments WHERE id=?').bind(commentDelMatch[2]).run();
      return json({ok:true});
    }
